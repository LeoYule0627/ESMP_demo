...
public interface TestRepository extends JpaRepository<Hcmio, Integer> {
    ...
    @Query(value = "CALL getDeliveryFeeDetail(?1, ?2, ?3)", nativeQuery = true)
    List<Map> getDeliveryFeeDetail(String branchNo, String custSeq, String date);

    @Query(value = "CALL getDeliveryFeeSum(?1, ?2, ?3)", nativeQuery = true)
    Map getDeliveryFeeSum(String branchNo, String custSeq, String date);

    @Query(value = "SELECT `type` FROM calendar WHERE `day`=?1", nativeQuery = true)
    String findByDay(String date);
}
